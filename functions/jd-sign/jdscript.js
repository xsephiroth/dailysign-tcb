...
var Key = process.env.JD_COOKIES; //单引号内自行填写您抓取的Cookie
...
function notify() {
    return new Promise((resolve) => {
        try {
            ...
            var Cash = merge.TotalCash.TCash
                ...
                try {
                    if (error) {
                        ...
                        if (cc.code == 3) {
                            ...
                            merge.JDTurn.bean +=
                                Number(cc.data.prizeSendNumber) || 0;
                            merge.JDTurn.notify += `${
                                also ? `\n` : ``
...
function JingRongBean(s) {
    return new Promise((resolve) => {
        ...
        setTimeout(() => {
            const login = {
                url:
                    "https://ms.jr.jd.com/gw/generic/zc/h5/m/queryOpenScreenReward",
                headers: {
                    Cookie: KEY,
                },
                body:
                    ...
                    if (error) {
                        ...
                        if (data.match(/\"login\":true/)) {
                            ...
                            if (cc.resultData.data.reward === false) {
                                await JRBeanCheckin(
                                    s,
                                    cc.resultData.data.rewardAmount
                                );
                            } else {
            ...
            $nobyda.post(JRSUrl, function (error, response, data) {
                try {
                    ...
                    if (data.match(/\"resBusiCode\":0/)) {
                        console.log("\n" + "京东金融-钢镚签到成功 " + Details);
                        const leng =
                            cc.resultData.resBusiData.actualTotalRewardsValue;
    ...
    return new Promise((resolve, reject) => {
        ...
        const JDUrl = {
            ...
            body: `body=${encodeURIComponent(
                `{"activityId":"${acData}"${
                    ask
                        ? `,"paginationParam":"2","paginationFlrs":"${ask}"`
                        : ``
                }}`
            )}`,
        };
                ...
                if (error) {
                    ...
                    const turnTableId =
                        ...
                        (od.floorList || [])
                            .filter(
                                (o) =>
                                    o.boardParams && o.boardParams.turnTableId
                            )
                    ...
                    if (data.match(/enActK/)) {
                        ...
                        let params =
                            ...
                            (od.floatLayerList || [])
                                .filter(
                                    (o) => o.params && o.params.match(/enActK/)
                                )
                                .map((o) => o.params)
                                .pop();
                        if (!params) {
                        ...
                        // 无签到数据, 但含有关注店铺签到
                        const boxds =
                            $nobyda.read("JD_Follow_disable") === "false"
                                ? false
                                : true;
                        if (boxds) {
                            console.log(`\n${title}关注店铺`);
                            ...
                            merge[
                                key
                            ].notify = `${title}: 失败, 需要关注店铺 ⚠️`;
                            merge[key].fail = 1;
                        }
                    } else if (page && !ask) {
                        // 无签到数据, 尝试带参查询
                        const boxds =
                            $nobyda.read("JD_Retry_disable") === "false"
                                ? false
                                : true;
                        if (boxds) {
                            console.log(`\n${title}二次查询`);
        ...
        (data) => {
            ...
            if (typeof data == "number")
                return JDUserSign2(s, key, title, data);
                    ...
                    if (error) {
                        ...
                        if (data.match(/签到成功/)) {
                            ...
                            if (data.match(/(已签到|已领取)/)) {
                                ...
                                merge[
                                    key
                                ].notify = `${title}: 失败, 原因: 活动已结束 ⚠️`;
                            } else if (data.match(/\"code\":\"?3\"?/)) {
                                merge[
                                    key
                                ].notify = `${title}: 失败, 原因: Cookie失效‼️`;
                            } else {
                                merge[
                                    key
                                ].notify = `${title}: 失败, 原因: 未知 ⚠️`;
                            }
                            merge[key].fail = 1;
                        }
                    }
...
function JDFlashSale(s) {
    return new Promise((resolve) => {
        ...
        setTimeout(() => {
            const JDPETUrl = {
                url:
                    "https://api.m.jd.com/client.action?functionId=partitionJdSgin",
                headers: {
                    Cookie: KEY,
                },
                body:
                    ...
                    if (error) {
                        ...
                        if (cc.result.code == 0) {
                            ...
                            if (data.match(/(已签到|已领取|\"2005\")/)) {
                                ...
                                data.match(/不存在|已结束|\"2008\"|\"3001\"/)
                            ) {
                                await FlashSaleDivide(s); //瓜分京豆
                                return;
                            } else if (
                                data.match(/(\"code\":\"3\"|\"1003\")/)
                            ) {
                                merge.JDFSale.notify =
...
function FlashSaleDivide(s) {
    return new Promise((resolve) => {
        setTimeout(() => {
            const Url = {
                url:
                    "https://api.m.jd.com/client.action?functionId=partitionJdShare",
                headers: {
                    Cookie: KEY,
                },
                body:
                ...
                try {
                    if (error) {
                        ...
                        if (cc.busiCode == "0") {
                            ...
                            merge.JDCash.Cash =
                                cc.result.signResult.signData.amount || 0;
                    ...
                    if (error) {
                        ...
                        if (data.match(/(\"name\":)/)) {
                            ...
                            if (data.match(/(\"name\":\"京豆\")/)) {
                                merge.JDCube.bean =
                                    cc.result.lotteryInfo.quantity || 0;
            ...
            $nobyda.get(JDkey, async function (error, response, data) {
                try {
                    if (error) {
                        ...
                        const Details = LogDetails ? "response:\n" + data : "";
                        if (
                            data.match(/\"raffleActKey\":\"[a-zA-z0-9]{3,}\"/)
                        ) {
                            const cc = JSON.parse(data);
                            merge.JDPrize.key =
                                cc.data.floorInfoList[0].detail.raffleActKey;
                            console.log(
                                "\n" + "京东商城-大奖查询成功 " + Details
                            );
                        ...
                        if (cc.message == "not login") {
                            ...
                            if (cc.data.task_status == 0 && cc.data.source_id) {
                                ...
                                const percent = Math.round(
                                    (cc.data.done_distance / cc.data.distance) *
                                        100
                                );
                                console.log(
                    ...
                    if (error) {
                        ...
                        if (cc.resultCode == 0) {
                            if (cc.resultData.data.businessData != null) {
                                if (
                                    cc.resultData.data.businessData
                                        .pickStatus == 2
                                ) {
                                    console.log(
                                        `\n${title}查询成功 ${Details}`
                                    ...
                                    if (
                                        data.match(/\"rewardPrice\":\"?(\d+)/)
                                    ) {
                                        var JRDoll_bean = data.match(
                                            /\"rewardPrice\":\"?(\d+)/
                                        )[1];
                                        await JingRongDoll(
                                            s,
                                ...
                                merge[
                                    key
                                ].notify = `${title}: 失败, 原因: 领取异常 ⚠️`;
                            }
                        } else {
                            console.log(`\n${title}签到失败 ${Details}`);
                            const redata =
                                typeof cc.resultData == "string"
                                    ? cc.resultData
                                    : "";
                    ...
                    if (error) {
                        ...
                        if (data.match(/\"type\":\d+?,/)) {
                            ...
                            if (data.match(/\"jdBeanAmount\":[1-9]+/)) {
                                merge.Overseas.bean = data.match(
                                    /\"jdBeanAmount\":(\d+)/
                                )[1];
            ...
            $nobyda.get(
                ...
                (error, response, data) => {
                    try {
                        ...
                        const Details = LogDetails ? "response:\n" + data : "";
                        const tm =
                            parseInt((Date.now() + 28800000) / 86400000) *
                                86400000 -
                            28800000;
        ...
        $nobyda.post(
            {
                ...
                headers: {
                    Cookie: KEY,
                    Referer:
                        "https://wqs.jd.com/my/jingdou/my.shtml?sceneval=2",
                },
            },
            (error, response, data) => {
                try {
...
function TotalCash() {
    return new Promise((resolve) => {
        ...
        $nobyda.post(
            {
                url:
                    "https://api.m.jd.com/client.action?functionId=myhongbao_balance",
                headers: {
                    Cookie: KEY,
                },
                body:
            ...
            (error, response, data) => {
                try {
                    ...
                    if (
                        ...
                        console.log("\n京东-总金贴查询成功 " + Details);
                        merge.TotalSubsidy.TSubsidy =
                            cc.resultData.data.balance || 0;
                    } else {
                        console.log("\n京东-总金贴查询失败 " + Details);
                    }
                } catch (eor) {
            ...
            (error, response, data) => {
                try {
                    ...
                    if (
                        ...
                        console.log("\n京东-总现金查询成功 " + Details);
                        merge.TotalMoney.TMoney =
                            cc.data.result.totalMoney || 0;
                    } else {
                        console.log("\n京东-总现金查询失败 " + Details);
                    }
                } catch (eor) {
...
function GetCookie() {
    try {
        if (
            $request.headers &&
            $request.url.match(/api\.m\.jd\.com.*=signBean/)
        ) {
            var CV = $request.headers["Cookie"];
            if (CV.match(/(pt_key=.+?pt_pin=|pt_pin=.+?pt_key=)/)) {
                var CookieValue =
                    CV.match(/pt_key=.+?;/) + CV.match(/pt_pin=.+?;/);
...
function nobyda() {
    ...
    // const NodeSet = "/tmp/CookieSet.json"; // TCB
    const node = (() => {
        if (isNode) {
